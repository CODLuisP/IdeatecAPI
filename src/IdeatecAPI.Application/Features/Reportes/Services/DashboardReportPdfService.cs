...
public class DashboardReportPdfService : IDashboardReportPdfService
{
    ...
    private string GenerarHtml(DashboardReportDto d)
    {
        ...
        {
            (i + 1).ToString(),
            c.NumeroCompleto,                       // número completo sin truncar
            Trunc(c.ClienteRznSocial, 32),          // solo el cliente se trunca
            c.FechaEmision.ToString("dd/MM/yyyy"),  // fecha completa
            Fmt(c.ImporteTotal)
        }).ToList();
...
""";
    ...
    private static string BuildVerticalBarChart(List<GraficoBarraDto> grafico)
    {
        ...
        // Períodos largos (mes/personalizado) traen un punto por día — agrupamos de 7 en 7
        // (semanas) para que el gráfico siga siendo legible en vez de amontonar 28-31 barras.
        ...
        if (maxV == 0) maxV = 1;
        var usarK = agruparSemanas || n > 7;   // abreviar solo si los montos suelen ser grandes
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            // Etiqueta inferior (día, rango de semana o nombre)
    ...
    {
        // Título en color sólido con texto blanco (misma intensidad que los KPI de arriba);
